...
class FOSTER(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._network = FOSTERNet(args['convnet_type'], False, args=args)
        self._snet = None
    ...
    def incremental_train(self, data_manager):
        ...
        # ----------------
        logging.info('\n[*] Fini! '.ljust(64, '-'))
        logging.info(f'[!] Training total time = {timedelta(seconds=end - start)}s')
        logging.info('Times:{}'.format(self.times))
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            ...
            if self.args['resume'] and osp.exists(self.model_repath):
                self.pre_model()
            else:
    ...
    def _feature_compression(self, train_loader, test_loader):
        self._snet = FOSTERNet(self.args['convnet_type'], False, args=self.args)
        ...
        # cnn_accy = self._evaluate(y_pred, y_true,self.args['init_cls'])
