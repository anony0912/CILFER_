...
class BaseLearner(object):
    def __init__(self, args):
        self.repth_root = args['repth_root']
        
        self.args = args
        self._cur_task = -1
        self._known_classes = 0
        self._total_classes = 0
        self._base_classes = args['init_cls']
        ...
        self.repth_confusion = osp.join(self.repth_root,'confusion_matrix/{}_{}_{}_{}_{}_{}_{}'.format(args['prefix'], args['seed'], args['model_name'], args['convnet_type'],
                                                args['dataset'], args['init_cls'], args['increment']))

        self.model_repath = osp.join(self.repth_root,'pth','{}_{}.pth'.format(args['model_name'],args['dataset']))
    @property
    ...
    def save_checkpoint(self):
        ...
        torch.save(save_dict, self.model_repath)
        logging.info('Save in {}...'.format(self.model_repath))
    
    def pre_model(self):
        print('Pretrained model load in incremental phase.,self._device:{}'.format(self._device))
        print("==> Loading pretrained weights: ", self.model_repath)
        pretrained = torch.load(self.model_repath,map_location=torch.device(self._device))
        ...
        self._network.load_state_dict(model_state_dict, strict = False)    
        self._network = self._network.to(self._device)
    
    def after_task(self):
        pass

    def _evaluate(self, y_pred, y_true,incremental_num):
        ret = {}
        grouped = accuracy(y_pred.T[0], y_true, self._known_classes,self._base_classes,increment=incremental_num)
    ...
    def eval_task(self, incremental_num,dict_name):
        y_pred, y_true = self._eval_cnn(self.test_loader)
        dict_name.update({'repth':self.repth_confusion})

        cnn_accy = self._evaluate(y_pred, y_true, incremental_num)
        
        if hasattr(self, '_class_means'):
            y_pred, y_true = self._eval_nme(self.test_loader, self._class_means)
    
            nme_accy = self._evaluate(y_pred, y_true,incremental_num)
        else:
            nme_accy = None
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        _class_means_ncm = np.zeros((self._total_classes, self.feature_dim))
        ...
        for class_idx in range(self._known_classes):
            ...
            _class_means_ncm[class_idx,:] = mean
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T
            class_mean = np.mean(vectors, axis=0)
            _class_means_ncm[class_idx,:] = class_mean
        ...
        self._class_means_ncm = _class_means_ncm
        
    
    
    def eval_flops_v1(self):
        inputs = torch.randn(1,3,224,224).to(self._device)
        flops, params = profile(self._network.to(self._device), inputs=(inputs, )) 
        total = sum([param.nelement() for param in self._network.parameters()])
        
        logging.info('[*] FLOPs = {:.2f} G'.format(flops/1000**3))
        logging.info('[*] Params = {:.2f} M'.format(params/1000**2))
        logging.info('[*] Total Params = {:.2f} M '.format(total/1000**2))
